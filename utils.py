...
def get_mean_and_std(dataset):
    ...
    for inputs, targets in dataloader:
        for i in range(3):
            mean[i] += inputs[:,i,:,:].mean() # (batch_num, channel, h, w)
            std[i] += inputs[:,i,:,:].std()
...
def init_params(net):
    ...
    for m in net.modules():
        if isinstance(m, nn.Conv2d): # if m belongs to nn.Conv2d class
            init.kaiming_normal(m.weight, mode='fan_out') # a way of initializing described in https://pytorch.org/docs/stable/nn.init.html, fan_in and fan_out can be found in https://pystyle.info/pytorch-parameters-initialization/
            if m.bias:
                init.constant(m.bias, 0) #Fills the input Tensor with the value \text{val}val.
        elif isinstance(m, nn.BatchNorm2d):
            init.constant(m.weight, 1) # gamma
            init.constant(m.bias, 0) # beta
...
def show_heatmap_on_image(img, mask):
    ...
    cam = heatmap + np.float32(img) # imposing heatmap on image
    cam = cam / np.max(cam)
...
def make_filter_id(net, verbose=False):
    layer_to_filter_id = {}
    ...
    for layer_num, (name, param) in enumerate(net.named_parameters()):
        ...
        if len(param.size()) == 4:
            for j in range(param.size()[0]):
                if (type(net) == type(torchvision.models.resnet18()) and ('conv' in name)) or (type(net) != type(torchvision.models.resnet18())):
    ...
    return layer_to_filter_id, filter_id_to_layer, conv_filter_id_to_layer, conv_idx, ind, conv_filter_num


def draw_graph(incorrect, correct, flipped, num_of_filter, path, figname):
    x = np.linspace(1, 26, num_of_filter)
    fig = plt.figure(figsize=(15, 4), dpi=360)

    ib = incorrect['baseline']
    ip = incorrect['proposed']
    ic = incorrect['conv5_x']
    ax2 = fig.add_subplot(131)
    ax2.plot(x, ib, label='Levin et al., 2021')
    ax2.plot(x, ip, label='POT')
    ax2.plot(x, ic, label='conv5_x')
    ...
    ax3.plot(x, cb, label='Levin et al., 2021')
    ax3.plot(x, cp, label='POT')
    ax3.plot(x, cc, label='conv5_x')
    ...
    ax1.plot(x, fb, label='Levin et al., 2021')
    ax1.plot(x, fp, label='POT')
    ax1.plot(x, fc, label='conv5_x')
    ax1.set_ylabel('percentage of corrected samples')
    ax1.set_xlabel('#finetuned filters')
    ax1.legend()

    name = os.path.join(path, figname) + '.png'
    fig.savefig(name)