...
def save_gradients(grads_to_save, args, experiment, reference_image, inv_transform_test):
    ...
    grads_to_save = np.abs(grads_to_save) # parameter saliency profile?
    # grads_to_save[grads_to_save < 0] = 0.0

    # what about changing the percentage of this? -> Is it getting closer to GradCAM?
...
def compute_input_space_saliency(reference_inputs, reference_targets, net, args, experiment,
    ...
    # reference_inputs : 入力画像
    # reference_targets : 入力画像に対応するクラス

    #First, log things
    with torch.no_grad():
        ref_image_to_log = inv_transform_test(reference_inputs[0].detach().cpu()).permute(1, 2, 0) # what is this for?


        reference_outputs = net(reference_inputs)
        _, reference_predicted = reference_outputs.max(1)
        # Log classes: <- displaying the first element of a batch? (maybe this function assumes one image as input)
        print("""\n
        Image target label: {}
    ...
    for noise_iter in range(args.noise_iters):
        ...
        #Backprop to the input <- perturbed_inputs are fed into the model and we get filter_saliency. We calculate Cosine_similarity and then backprop to perturbed_inputs
        ...
        #Boost them: <- boost_factor is the large constant whose robustness needs to be inspected.
        filter_saliency_boosted = filter_saliency.detach().clone()
        filter_saliency_boosted[sorted_filters[:args.k_salient]] *= args.boost_factor # extracting top-k salient filters
...
def sort_filters_layer_wise(filter_profile, layer_to_filter_id, filter_std = None):
    ...
    for layer in layer_to_filter_id:
        ...
        means.append(np.ones_like(filter_profile[layer_inds])*np.mean(filter_profile[layer_inds])) #layer_wise mean value of saliency
...
if __name__ == '__main__':
    ...
    if images_path == 'path_to_imagenet':
    ...
    #Load inference files <- inference results on ImageNet valset
    ...
    #Load valset stats files <- parameter saliency mean and std for standardization
    ...
    if args.reference_id is None:
        ...
        # default image_path is raw_images/great_white_shark_mispred_as_killer_whale.jpeg
        # image_target_label is number from 0 to 999 according to ImageNet labels
        reference_image = transform_raw_image(args.image_path).unsqueeze(0) # normalize the image, unsqueeze is to enable us to treat an image and a batch as the same thing.
